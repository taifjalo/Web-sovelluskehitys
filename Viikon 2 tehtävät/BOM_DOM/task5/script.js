...
// first we choose the location.coordinates which is index 0 in the restaurants array to center the map
// create map centered at the first restaurant's coordinates
const firstCoords = restaurants[0].location.coordinates; // [lon, lat]
const map = L.map('map').setView([firstCoords[1], firstCoords[0]], 6); // [lat, lon]
...
restaurants.forEach(rest => {
  const coords = rest.location.coordinates; // [lon, lat]
  const marker = L.marker([coords[1], coords[0]]).addTo(map); // [lat, lon]
